-- 공지권한
CREATE TABLE `MY_SCHEMA`.`nm_notice_authority` (
...
-- 공지권한
ALTER TABLE `MY_SCHEMA`.`nm_notice_authority`
...
-- 공지사항게시글
CREATE TABLE `MY_SCHEMA`.`nm_notice_board` (
...
-- 공지사항게시글
ALTER TABLE `MY_SCHEMA`.`nm_notice_board`
...
-- 공지사항댓글
CREATE TABLE `MY_SCHEMA`.`nm_notice_reply` (
...
-- 공지사항댓글
ALTER TABLE `MY_SCHEMA`.`nm_notice_reply`
...
-- 공지사항사진
CREATE TABLE `MY_SCHEMA`.`nm_notice_photo` (
...
-- 공지사항사진
ALTER TABLE `MY_SCHEMA`.`nm_notice_photo`
...
-- 등급
CREATE TABLE `MY_SCHEMA`.`nm_grade` (
  `grade_no`   INTEGER      NOT NULL COMMENT '등급번호', -- 등급번호
...
-- 등급
ALTER TABLE `MY_SCHEMA`.`nm_grade`
...
-- 메시지
CREATE TABLE `MY_SCHEMA`.`nm_message` (
...
-- 메시지
ALTER TABLE `MY_SCHEMA`.`nm_message`
...
-- 모임
CREATE TABLE `MY_SCHEMA`.`nm_group` (
  `group_no`       INTEGER      NOT NULL COMMENT '모임번호', -- 모임번호
...
-- 모임
ALTER TABLE `MY_SCHEMA`.`nm_group`
...
-- 모임회원
CREATE TABLE `MY_SCHEMA`.`nm_meeting_member` (
  `group_no`  INTEGER NOT NULL COMMENT '모임번호', -- 모임번호
  `member_no` INTEGER NOT NULL COMMENT '회원번호', -- 회원번호
  `grade_no`  INTEGER NOT NULL COMMENT '등급번호' -- 등급번호
)
...
-- 모임회원
ALTER TABLE `MY_SCHEMA`.`nm_meeting_member`
...
-- 사용권한
CREATE TABLE `MY_SCHEMA`.`nm_write_authority` (
  `authority_no`   INTEGER     NOT NULL COMMENT '사용권한번호', -- 사용권한번호
...
-- 사용권한
ALTER TABLE `MY_SCHEMA`.`nm_write_authority`
...
-- 소통게시글
CREATE TABLE `MY_SCHEMA`.`nm_communication_board` (
...
-- 소통게시글
ALTER TABLE `MY_SCHEMA`.`nm_communication_board`
...
-- 소통게시글댓글
CREATE TABLE `MY_SCHEMA`.`nm_communication_reply` (
...
-- 소통게시글댓글
ALTER TABLE `MY_SCHEMA`.`nm_communication_reply`
...
-- 소통게시글사진
CREATE TABLE `MY_SCHEMA`.`nm_communication_photo` (
...
-- 소통게시글사진
ALTER TABLE `MY_SCHEMA`.`nm_communication_photo`
...
-- 소통게시판좋아요
CREATE TABLE `MY_SCHEMA`.`nm_communication_board_like` (
  `member_no`              INTEGER NOT NULL COMMENT '회원번호', -- 회원번호
  `communication_board_no` INTEGER NOT NULL COMMENT '소통게시글번호' -- 소통게시글번호
)
COMMENT '소통게시판좋아요';
...
-- 소통게시판좋아요
ALTER TABLE `MY_SCHEMA`.`nm_communication_board_like`
...
-- 알림
CREATE TABLE `MY_SCHEMA`.`nm_alram` (
  `member_no`       INTEGER NOT NULL COMMENT '회원번호', -- 회원번호
...
-- 알림
ALTER TABLE `MY_SCHEMA`.`nm_alram`
...
-- 일정
CREATE TABLE `MY_SCHEMA`.`nm_plan` (
...
-- 일정참여자
CREATE TABLE `MY_SCHEMA`.`nm_plan_member` (
  `plan_board_no` INTEGER NOT NULL COMMENT '일정번호', -- 일정번호
  `member_no`     INTEGER NOT NULL COMMENT '회원번호', -- 회원번호
  `group_no`      INTEGER NOT NULL COMMENT '모임번호' -- 모임번호
)
...
-- 일정참여자
ALTER TABLE `MY_SCHEMA`.`nm_plan_member`
...
-- 피드게시글
CREATE TABLE `MY_SCHEMA`.`nm_feed` (
...
-- 피드게시글댓글
CREATE TABLE `MY_SCHEMA`.`nm_feed_reply` (
...
-- 피드게시글댓글
ALTER TABLE `MY_SCHEMA`.`nm_feed_reply`
...
-- 피드게시글사진
CREATE TABLE `MY_SCHEMA`.`nm_feed_photo` (
...
-- 피드게시글사진
ALTER TABLE `MY_SCHEMA`.`nm_feed_photo`
...
-- 회계
CREATE TABLE `MY_SCHEMA`.`nm_account` (
...
-- 회계
ALTER TABLE `MY_SCHEMA`.`nm_account`
...
-- 회계권한
CREATE TABLE `MY_SCHEMA`.`nm_account_authority` (
...
-- 회계권한
ALTER TABLE `MY_SCHEMA`.`nm_account_authority`
...
CREATE UNIQUE INDEX `UIX_nm_account_authority`
  ON `MY_SCHEMA`.`nm_account_authority` ( -- 회계권한
    `group_no` ASC,     -- 모임번호
...
-- 회계유형
CREATE TABLE `MY_SCHEMA`.`nm_account_type` (
  `account_type_no`   INTEGER      NOT NULL COMMENT '회계유형번호', -- 회계유형번호
...
-- 회계유형
ALTER TABLE `MY_SCHEMA`.`nm_account_type`
...
-- 회원
CREATE TABLE `MY_SCHEMA`.`nm_member` (
  `member_no`      INTEGER      NOT NULL COMMENT '회원번호', -- 회원번호
...
-- 회원
ALTER TABLE `MY_SCHEMA`.`nm_member`
...
ALTER TABLE `MY_SCHEMA`.`nm_notice_authority`
  ADD CONSTRAINT `FK_nm_write_authority_TO_nm_notice_authority` -- 사용권한 -> 공지권한
    ...
    REFERENCES `MY_SCHEMA`.`nm_write_authority` ( -- 사용권한
      `authority_no` -- 사용권한번호
    );
...
ALTER TABLE `MY_SCHEMA`.`nm_notice_authority`
  ADD CONSTRAINT `FK_nm_grade_TO_nm_notice_authority` -- 등급 -> 공지권한
    ...
    REFERENCES `MY_SCHEMA`.`nm_grade` ( -- 등급
      `grade_no` -- 등급번호
    );
...
ALTER TABLE `MY_SCHEMA`.`nm_notice_authority`
  ADD CONSTRAINT `FK_nm_group_TO_nm_notice_authority` -- 모임 -> 공지권한
    ...
    REFERENCES `MY_SCHEMA`.`nm_group` ( -- 모임
      `group_no` -- 모임번호
    );
...
ALTER TABLE `MY_SCHEMA`.`nm_notice_board`
  ADD CONSTRAINT `FK_nm_meeting_member_TO_nm_notice_board` -- 모임회원 -> 공지사항게시글
    ...
    REFERENCES `MY_SCHEMA`.`nm_meeting_member` ( -- 모임회원
      `group_no`,  -- 모임번호
      `member_no`  -- 회원번호
    );
...
ALTER TABLE `MY_SCHEMA`.`nm_notice_reply`
  ADD CONSTRAINT `FK_nm_notice_board_TO_nm_notice_reply` -- 공지사항게시글 -> 공지사항댓글
    ...
    REFERENCES `MY_SCHEMA`.`nm_notice_board` ( -- 공지사항게시글
      `notice_board_no` -- 공지사항번호
    );
...
ALTER TABLE `MY_SCHEMA`.`nm_notice_reply`
  ADD CONSTRAINT `FK_nm_member_TO_nm_notice_reply` -- 회원 -> 공지사항댓글
    ...
    REFERENCES `MY_SCHEMA`.`nm_member` ( -- 회원
      `member_no` -- 회원번호
    );
...
ALTER TABLE `MY_SCHEMA`.`nm_communication_reply`
  ADD CONSTRAINT `FK_nm_communication_board_TO_nm_communication_reply` -- 소통게시글 -> 소통게시글댓글
    ...
    REFERENCES `MY_SCHEMA`.`nm_communication_board` ( -- 소통게시글
      `communication_board_no` -- 소통게시글번호
    );
...
ALTER TABLE `MY_SCHEMA`.`nm_alram`
  ADD CONSTRAINT `FK_nm_plan_TO_nm_alram` -- 일정 -> 알림
    ...
    REFERENCES `MY_SCHEMA`.`nm_plan` ( -- 일정
      `plan_board_no` -- 일정번호
    );
...
ALTER TABLE `MY_SCHEMA`.`nm_alram`
  ADD CONSTRAINT `FK_nm_feed_TO_nm_alram` -- 피드게시글 -> 알림
    ...
    REFERENCES `MY_SCHEMA`.`nm_feed` ( -- 피드게시글
      `feed_no` -- 피드게시글번호
    );
...
ALTER TABLE `MY_SCHEMA`.`nm_account`
  ADD CONSTRAINT `FK_nm_account_type_TO_nm_account` -- 회계유형 -> 회계
    ...
    REFERENCES `MY_SCHEMA`.`nm_account_type` ( -- 회계유형
      `account_type_no` -- 회계유형번호
    );
